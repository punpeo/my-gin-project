...
type ProductInfo struct {
	...
	CreatedAt    time.Time `gorm:"not null;index" json:"created_at"`                      // 创建时间，非空+索引（按时间筛选/排序高频）
	UpdatedAt    time.Time `gorm:"not null;index" json:"updated_at"`                      // 更新时间，非空+索引（按更新时间筛选/排序高频）
}
