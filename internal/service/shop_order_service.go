...
type ShopOrderStat struct {
	...
	TotalNum int     // 订单数量总和（C列）
	TotalAmt float64 // 订单金额总和（D列）
}

// StatShopOrder 遍历指定目录下所有Excel文件，统计店铺订单数据
// 原有逻辑保持不变
func StatShopOrder(basePath string) ([]*ShopOrderStat, *excelize.File, int, float64, error) { // 新增返回总订单数、总金额
	// 1. 校验目录是否存在
	_, err := os.Stat(basePath)
	if os.IsNotExist(err) {
		return nil, nil, 0, 0, fmt.Errorf("文件根目录不存在：%s", basePath)
	}
	...
	if err != nil {
		return nil, nil, 0, 0, fmt.Errorf("遍历目录失败：%v", err)
	}

	if len(excelFiles) == 0 {
		return nil, nil, 0, 0, fmt.Errorf("指定目录下未找到xlsx/xls格式文件")
	}
	...
	for _, file := range excelFiles {
		...
		for i, row := range rows {
			...
			if err != nil {
				fmt.Printf("警告：文件%s第%d行C列订单数量格式错误（%s），跳过\n", file, i+1, numStr)
				continue
			}
			...
			if err != nil {
				fmt.Printf("警告：文件%s第%d行D列订单金额格式错误（%s），跳过\n", file, i+1, amtStr)
				continue
			}
	...
	if len(statMap) == 0 {
		return nil, nil, 0, 0, fmt.Errorf("未统计到有效店铺订单数据")
	}
	...
	if err != nil {
		return nil, nil, 0, 0, fmt.Errorf("创建数字格式失败：%v", err)
	}
	...
	// 返回总计数据
	return statList, newFile, totalAllNum, totalAllAmt, nil
}

// StatShopOrderByUpload 通过上传的文件统计店铺订单数据
// 前端只会上传一个文件
func StatShopOrderByUpload(fileContent []byte, fileName string) ([]*ShopOrderStat, *excelize.File, int, float64, error) {
	// 1. 验证文件格式
	ext := filepath.Ext(fileName)
	if ext != ".xlsx" && ext != ".xls" {
		return nil, nil, 0, 0, fmt.Errorf("文件格式不支持，仅支持.xlsx和.xls格式")
	}
	...
	if err != nil {
		return nil, nil, 0, 0, fmt.Errorf("解析上传文件失败：%v", err)
	}
	defer f.Close()
	...
	rows, err := f.GetRows(f.GetSheetName(0))
	if err != nil {
		return nil, nil, 0, 0, fmt.Errorf("读取工作表数据失败：%v", err)
	}

	if len(rows) == 0 {
		return nil, nil, 0, 0, fmt.Errorf("上传文件无数据")
	}
	...
	for i, row := range rows {
		...
		num, err := strconv.Atoi(numStr)
		if err != nil {
			fmt.Printf("警告：上传文件第%d行C列订单数量格式错误（%s），跳过\n", i+1, numStr)
			continue
		}
		...
		amt, err := strconv.ParseFloat(amtStr, 64)
		if err != nil {
			fmt.Printf("警告：上传文件第%d行D列订单金额格式错误（%s），跳过\n", i+1, amtStr)
			continue
		}
